...
4. **实现功能后必须验证本功能测试全部通过，再验证全部功能测试通过**：每次实现功能之后，首先确保本功能的测试用例全部通过，然后运行全部功能测试用例（`c4/test/` 下所有测试套件）确保没有引入回归。本功能未全部通过前不得进入下一步，全部测试未通过前不得提交。当所有的功能测试用例全部通过后，将 `c4_function.md` 对应的功能条目设置成已完成（标题后加 ✅）。
