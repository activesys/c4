...
// validate_points 工具（C4_FUN_00088）——契约: agent.md §2.7.1；规则: c4_asfp2_client.md §validate_points。
...
func validateASFPoints(points []clientPoint) []asfpIssue {
	issues := []asfpIssue{}
	...
	for _, pt := range points {
		...
		if prev, ok := seen[pt.Addr]; ok {
			issues = append(issues, asfpIssue{Code: "POINT_DUP",
				Message: fmt.Sprintf("点 %s 与 %s 的 addr（ASFP2 key）=%d 重复——发送映射 构建时后者静默覆盖前者", prev, id, pt.Addr),
