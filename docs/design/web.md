...
> **关键词**触发（见 §3.1.3），前端设计以关键词驱动为准，**不依赖 interrupt 事件**。
...
Agent 生成接入方案后需要用户确认。**后端唯一真实的确认机制是关键词**，前端据此渲染确认按钮：

**后端关键词识别**（`C4Agent` 内两套独立正则）：
- 确认正则：`/确认|好的|执行|按方案|开始/` → 命中后注入上下文并执行方案。
- 拒绝正则：`/取消|拒绝|放弃|停止|算了|不执行|不要执行|不确认/` → 用于反向防误判。

**前端确认按钮**（「确认显性化」落点）：
1. 前端在**累积缓冲**（当前 Agent 气泡的完整文本）上匹配方案展示句式（如「是否确认」
   「确认执行」），命中后在输入区上方渲染常驻的「确认 / 取消」按钮。
2. 「确认」→ 发起普通 POST `{ message:"确认", history }`；「取消」→ POST `{ message:"取消，不执行", history }`。
   两者都是**新的一轮对话**，不依赖任何 interrupt/resume 机制。

> **匹配健壮性**（LLM token 非确定性）：
> - 匹配对象是**完整累积文本**而非单个 token 事件——「是否」+「确认」可能被拆成两个 text 事件。
> - 匹配**完整句式**而非单词——「执行」「好的」「开始」等词在普通语句中极易误触发，
>   仅在检测到「是否确认」「确认执行」等明确句式时才渲染按钮。
...
- **会话关联**：upload 接口**不接收也不返回 `conversationId`**，其 SSE 事件亦不带该字段，
  **无法与进行中的对话做服务端关联**。上传是「一次性解析、结果纯文本回显」——如需让解析结果
  参与后续对话，由前端把回显文本自行存入本地 `history`，在**下一轮** `POST /api/chat` 时随
  `history` 回传即可。
...
└── src/
    ...
    └── components/
        ...
        ├── ConfirmButtons.tsx          # 方案确认按钮（关键词驱动，§3.1.3）
...
| 方案确认 | interrupt 卡片 / 关键词驱动 | 关键词驱动（唯一） | 后端声明 interrupt 但从不产出，关键词是唯一真实机制 |
