...
> **版本**：v0.1.0 | **最后更新**：2026-07-17 | **父文档**：[c4_architecture.md](c4_architecture.md) | **对应功能**：[C4_FUN_00042](../specification/c4_function.md), [C4_FUN_00057](../specification/c4_function.md)
...
扩容/调整阶段：
  9. Agent 执行 Pause-Resume 协议：
     a. Agent 向 c4_asfp2_server 发送 `pause` → 停止数据接收
     b. Agent 调用 c4_shm_manager.adjust_shm()
     c. Agent 向 c4_asfp2_server 发送 `resume`
        → server 重新读配置 → 对比实例 id → 先停后启 → 重建索引 → 返回
```
...
sequenceDiagram
    ...
    S-->>A: 工具列表（pause / resume / status / start）
    ...
    SM-->>A: 完成
    A->>S: resume()
    S->>S: remap_version 变化 → munmap+mmap
    S->>S: 重新读配置 → 对比实例 → 重建索引
    S-->>A: "success"
```
...
收到 Agent 的 `resume` 指令后，`c4_asfp2_server` 执行以下步骤：

1. 检测 `remap_version` 变化 → `munmap` + `mmap` 重新映射共享内存
2. 重新读取配置文件中的 `c4_asfp2_server` 段
3. 以 `id` 为唯一标识符，对比新旧实例列表，按**先停后启**的顺序执行：

   **Phase 1 — 停止**（先处理所有需停止的实例，释放端口）：
   - 旧配置中存在、新配置中**不存在**的 `id` → 关闭对应 listener，等待现有连接自然结束
   - 旧配置中存在、新配置中也存在但 **port 已变更**的 `id` → 同上，关闭旧 listener

   **Phase 2 — 启动**（所有释放完成后，启动新实例）：
   - 新配置中存在、旧配置中**不存在**的 `id` → 启动新 goroutine 监听对应端口
   - 旧配置中存在、新配置中也存在但 **port 已变更**的 `id` → 在新端口启动 goroutine
   - 等待 Phase 2 所有新增/变更实例的 `net.Listen` 全部成功后，再进入 Phase 3

   **Phase 3 — 更新**（不涉及端口变更的实例原地更新）：
   - 新、旧配置中 `id` 相同且 `port` 不变 → 仅更新 addr → shm_id 映射

4. 重建 addr → shm_id 索引
5. 恢复数据接收
6. 确认所有实例均已监听后，MCP 应答 resume 成功

> **端口复用安全**：先停后启的顺序保证了即使新实例复用了被删除实例的端口，也不会出现端口占用冲突。
...
首次初始化：加载配置文件、附加共享内存、启动所有 ASFP2 Server goroutine。
**仅在服务刚启动、尚未调用过 `start` 时有效**。调用成功后 `start` 不再可用，
后续配置变更通过 `resume` 执行。
...
| 错误码 | 含义 |
|--------|------|
| `ALREADY_STARTED` | `start` 已调用过，服务已处于运行状态 |
...
暂停所有 Server goroutine 的数据接收。已建立的 TCP 连接保持但暂停解析和写入。
**若 `start` 未成功调用过，返回 `SERVICE_NOT_READY`。**
    ...
    "inputSchema": {
        ...
        "properties": {
            "shm_ids": {
                ...
                "description": "需要暂停的 shm_id 列表。空数组表示暂停全部实例"
            }
        },
        "required": ["shm_ids"]
    }
...
恢复数据接收（配置重载）。检测 `remap_version` 变化后自动重映射，通过 `roots/list`
重新获取配置路径并读取配置，按 §3.3 的流程对比新旧实例列表、先停后启、重建 addr → shm_id 索引。
**若 `start` 未成功调用过，返回 `SERVICE_NOT_READY`。**
...
```json
{
    ...
    "inputSchema": {
        ...
        "properties": {
            ...
            "new_max_points": {
                "type": "integer",
                "description": "扩容后的新 max_points 值"
            }
        },
        "required": ["shm_ids"]
    }
...
查询各 Server 实例的运行状态和数据统计。**若 `start` 未成功调用过，返回 `SERVICE_NOT_READY`。**
...
| 场景 | 触发工具 | 处理方式 |
|------|---------|---------|
| `start` 已调用后再次调用 `start` | `start` | 返回 `ALREADY_STARTED` |
| `start` 未完成前调用 `pause`/`resume`/`status` | 各工具 | 返回 `SERVICE_NOT_READY` |
| `roots/list` 超时或路径缺失 | `start` / `resume` | 返回 `CONFIG_PATH_MISSING` |
| 配置文件格式错误 | `start` / `resume` | 返回 `isError: true` + `CONFIG_PARSE_ERROR` |
| 端口重复（配置冲突） | `start` / `resume` | 返回 `isError: true` + `PORT_CONFLICT` |
| 单个端口被占用（非配置冲突） | `start` / `resume` | 返回 `isError: true` + 具体端口；不影响其余实例 |
| 共享内存 magic 校验失败 | `start` / `resume` | 返回 `SHM_CORRUPTED`，Agent 应重建共享内存后重试 |
...
> **对应功能**：C4_FUN_00042, C4_FUN_00057
>
