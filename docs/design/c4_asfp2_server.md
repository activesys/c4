...
| `port` | int | 首个实例从 `9000` 开始，后续递增 | ASFP2 服务端监听端口，每个实例必须唯一。**端口由 Agent 在生成实例配置时确定性分配**（见 agent.md「监听端口的确定性分配」）：从默认 `9000` 起顺序 +1，直到找到未被占用的端口；占用 = ① `config.json` 中同服务已有实例声明的端口 ② 操作系统当前处于 LISTEN 状态的端口（含外部应用）。**已接入实例的端口永不变更**——在其上加点/修改时端口保持原值；用户显式指定端口时以用户指定为准 |
