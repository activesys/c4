...
**操作粒度**：`stop` 和 `start` 的操作对象是整个 MCP 服务进程。`stop` 关闭所有数据路径并销毁实例状态，`start` 重新加载配置并启动全部实例。共享内存级别的调整由 `c4_shm_manager` 的 `adjust_shm` 工具负责。
...
// ========== 成功 ==========
// --> 请求
{"jsonrpc": "2.0", "id": 11, "method": "tools/call", "params": {"name": "start", "arguments": {}}}
// <-- 应答
