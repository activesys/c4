...
3. **完备性裁决归代码**：提取结果与必填清单的差异、合法范围、身份查重、跨字段约束的
   ...
   一次做对的指导，不是校验器。
4. **单缺口顺序提问**：缺口由代码按依赖序计算，每回合只提问最靠前的一个未闭合缺口，
   提问后回合即终止（提问即终局，§2.6）。不允许 LLM 自问自答；流程行为不依赖重试循环矫正。
...
③ 状态合并：按「单调累积 + 显式改口才覆盖」规则并入会话状态；协议受逐侧锁约束
  ↓
④ 缺口计算：按依赖序检查各阶段出口判据；取依赖序最靠前的一个未闭合缺口
  ↓
⑤ 分叉：
   有缺口 → 单缺口提问（一次只问一项，提问即终局）→ 等待下一条消息
   无缺口 → 确定性穿越 8 → 展示方案 → button_arm → 停等确认
```

信息充分的用户（一条消息给全场站/协议/点表/转发/连接）零追问直达方案展示；
信息缺失时每个缺口在所属阶段被拦下，按依赖序逐项提问、逐项闭合。**不做聚合提问的
理由：协议决定点表与连接的问题内容**（modbus 点表要功能码/类型，asfp2 只要
地址+点名；modbus 连接要 ip/port/从站号，asfp2 只要监听端口）——协议未闭合前把
下游缺口一并列出，问题本身无法确切生成，用户只能面对含糊的清单。已闭合项在提问前
以 recap 复述（隐式确认）。运行时轮次由用户输入的完备度决定，与阶段编号无关。
...
- **单缺口顺序**：缺口计算器按依赖序取**最靠前的一个**未闭合缺口生成提问，提问后
  回合即终止，等待用户答复——不允许自问自答，不允许问完继续调用方案/执行类动作；
  用户答复闭合后顺序推进下一项。已闭合项在提问前以 recap 复述（"本次接入目前已确认"），
  构成隐式确认；
- **确切格式引导**：问题文本由 registry schema 生成（`gap_question` 模块），区分
  **要求**与**示例**——要求 = 字段完备性（schema 必填字段清单，缺了走字段级缺口
  追问，如「点『3000』缺 功能码」）；格式 = 示例（"写法不限（冒号、逗号、空格分隔
  均可），例如：3000:风速:1:3:4:0"）。**解析宽容不变**：不引入书写格式校验器，
  松散文本照常由提取器解析（「2391是正向有功」混述即为例证）；点表大时提示可直接
  上传 xlsx/csv 文件（`doc_parsers` 通道）。格式引导提高回答质量，不构成拒绝理由；
...
- **disarm 条件**：方案被消耗（**执行完成**）、被新方案覆盖（方案过期）、缺口置位
  （单缺口提问停等）、出口判据拒绝——但**若仍存在未消耗的有效方案则重发 `button_arm`**（拒绝
...
（引用其翻译后的最终 id 与显示名；确定性映射，不询问、不编造；引用不落盘，转发点配置不含 name）。
方案展示不得把采集/转发两侧点表作为孤立列表分别罗列——
转发点须逐条展示与采集点的对应：`采集 <地址>（<点名>） → 转发 <地址>`。
采集/转发点表必须数量相等才能建立一一对应；数量不等属点表错误——询问用户澄清修正
（不截断、不自动补齐、不生成点名），修正后方可进入方案确认。

**询问 vs 确认分离**：询问（提取层，补齐缺失信息，如「请提供 IP」）≠ 确认
（方案层，批准方案，如「是否执行？」按钮）。必填字段通过提取层单缺口顺序提问逐项收集，
...
> 自行编造默认值（C4_RS_00044）。缺失引导由提取层单缺口顺序提问负责（§2.6），
...
| 用户未提供点名（仅地址/序号） | 询问用户；**仍不提供 → 拒绝接入**（点名必填，系统不自动生成点名） |

> **两侧点名必要性（采集 vs 转发）**：本表适用于**采集点**——采集点点名**必填**（用户提供，未提供即询问；
> 仍不提供 → **拒绝接入**）。**转发点无自身点名**：即使转发点表自带点名也**忽略**；展示与运行期推导时
> **按序引用对应采集点的点名**（引用翻译后的最终 id 与显示名；确定性映射、不询问、不编造、不适用 `p_` 生成）。
> 引用≠落盘——转发点配置不含 `name` 字段（经 key 解析采集点即得，落盘属冗余）。

> **空字符串视为无点名**：点名缺失与空字符串等价，二者都归入「未提供点名」态，走询问路径（仍不提供 → 拒绝接入）。
> **身份字段与查重**：`point_schema.identity_fields` 声明能唯一标识一个点的字段（按字段名，顺序即
> 拼接顺序，独立于 `fields` 声明顺序）——如 asfp2_server 为 `["addr"]`，modbus_client 为 `["uid", "fun", "addr"]`。
> 身份字段组合用于点查重（见下方硬约束），**不再用于生成点名**——自动生成点名机制已废止：采集点点名必须
> 由用户提供（或由用户提供的中文名翻译而来），缺失即拒绝接入；转发点点名按序引用采集点点名（不落盘）。
> 字段值本身仍可能被采集时配错，故仍须方案确认展示映射核对。查重全程由 registry 驱动，
> 零协议硬编码；点重复同样报告用户。身份字段组合唯一同时意味着同一地址仅允许一种
> 解码配置——如 Modbus 同 uid/fun/addr 配不同 type/swap 即点重复（禁止同一寄存器双类型解码）。
> 转发端（Reader，如 InfluxDB）无自身点名——按序引用对应采集点点名，不生成、不落盘。

**硬约束（确定性兜底，不依赖 LLM 自觉）**：无论点名来自用户原文还是翻译，写入 config.json
...
4. 违规处理：格式不符 → 报告用户并要求提供合规英文点名（不自动替换生成）；长度超限 → 报错，提示用户「点名太长，需 1K 以内」；点重复（identity_fields 组合重复，无论 id 来源）→ 报告用户，让用户选择「提供新点表」或「结束本次接入任务」（报告口径见下注）
5. 落盘完整性：采集点写入 config.json 时必须同时含 `points[i].name`（用户原点名，原样保存，可为中文）与
   `points[i].id`（英文标识，见上）——缺任一即拦截，不得只写 id；转发点**不含 `name` 字段**——只含
   `points[i].key`（引用采集点全局 key），点名经 key 解析采集点即得，落盘 name 属冗余数据
...
> /off-by-one）对所有 id 都交由方案确认的逐条映射展示核对——id 均源自用户原文或翻译，
> 名与值都可能错。
...
> 示例为信息充分的乐观路径。信息缺失时对应阶段记缺口 → 按依赖序单缺口提问 → 停等，
