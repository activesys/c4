...
> 便捷通道仅限模板句式；模板外的协议名答复不构成声明，须经一轮是非题确认（设计依据：转发侧
> 声明通道刻意收窄——自然语言不可枚举，确定性由结构判定保证，见 func_test_case.md 用例 14
> 修复记录）。两类协议都确定后信息才算收集齐。
...
对每个 ServiceStep：
  ...
  action = "modify":
    ...
    2. 用 AccessPlanSteps 中的字段覆盖匹配实例的对应字段（浅合并）；
       例外：`port` 不参与覆盖——已接入实例的端口保持原值（监听端口的必填约束）
...
> - **已接入实例**：端口一经确定**永不变更**——在其上加点/修改配置时端口保持原值。
