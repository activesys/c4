...
Flag：
ASFP2标志位是8个字节，前四个字节为协议标识符 `ASFP`，后四个字节为版本号（格式 `V<major><minor><patch>`）。例如 ASFP2.0.0 的 Flag 为 `ASFPV200`，ASFP2.1.0 为 `ASFPV210`，ASFP2.1.1 为 `ASFPV211`，ASFP2.1.8 的 Flag 为 `ASFPV218`。

> **重要**：从 2.1.1 版本起，协议**仅支持 `ASFP` 标识符**。收到非 `ASFP` 标识符（如 `GSFP`、`GYFP` 等）应作为异常处理，不存在 `GSFPV211`、`GYFPV211` 等 Flag 字符串。
Length：
...
**2.1.1 起**，实现方在收发 FLOAT 类型时必须根据对端 Flag 中的版本号决定行为：

| 对端版本（Flag） | 发送端行为 | 接收端行为 |
|----------|-----------|-----------|
| < 2.1.1（`ASFPV200`、`ASFPV210`） | 本机序直接传输，**不做**字节 swap | 本机序直接解读，**不做**字节 swap |
| ≥ 2.1.1（`ASFPV211`） | 转为网络序（大端）后传输 | 从网络序（大端）转为本机序后解读 |
...
从 ASFP2 2.1.1 起，协议**仅支持 `ASFP` 标识符**。

实现方应确保：
- 接收时仅识别 `ASFP` 标识符，收到 `GSFP`、`GYFP` 等非 `ASFP` 标识符应作为无效 Flag 异常处理
- 发送版本由调用方通过 Flag 决定（`ASFPV200` / `ASFPV210` / `ASFPV211`），实现方应同时支持三种版本的编码，以便与不同版本对端互通
- `GSFPV211`、`GYFPV211` 不存在，实现方不应生成或期望这些 Flag
