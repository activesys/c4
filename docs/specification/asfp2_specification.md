...
数据项中的type，key，timestamp，status是由Attribute字段决定是否出现在数据项中的，但是value必须出现，因为它表示实际的数据。Data中的各个字段都是网络序的。
...
但是这两个类型还有一个特殊情况，当ASFP2_ATTRIBUTE_KEY_SEQUENCE, ASFP2_ATTRIBUTE_SAME_DATA_TYPE, ASFP2_ATTRIBUTE_SAME_TIMESTAMP，ASFP2_ATTRIBUTE_SAME_STATUS四个属性都有效的时候，采用压缩的模式，每个数据占用一位，数据是字节对齐的，每个字节都是从地位到高位编号的，最后一个字节的高位填充0。
...
浮点数类型
包括ASFP2_TYPE_FLOAT16, ASFP2_TYPE_FLOAT32, ASFP2_TYPE_FLOAT64，这些类型也没有特殊之处，按照各自的长度进行传输，由于浮点数都是IEEE745标准的，所以不存在本机序和网络序。
