...
- `value` 为 8 字节大端存储，按原类型解析：
  ...
  - FLOAT32 → `struct.unpack("f", value[4:8])`（v2.0.0/v2.1.0 为本机序，v2.1.1 才用 `>f`）
...
- asfp2_client 仅支持 v2.0.0 和 v2.1.0（`-P 7`），不支持 v2.1.1（FLOAT 网络序）。
  如需测试 v2.1.1，需另外构造包含 `"ASFPV211"` Flag 的原始 TCP 数据包。
