...
## 4. 过程健康度断言（对应 agent.md §2.4 回合状态机强化）

> 背景：9/12~16 全量回归中，用例 31 单轮按钮 7 连弹、"执行完成 ✅" 后爆出「设备信息整理环节
> 没有真正完成」的假成功、连续 5 轮空转等过程缺陷全部漏检——shm 值达标即 PASS 的判定对过程
> 缺陷是盲的。本节断言升级后，**过程违规即 FAIL**，与最终值断言并列。

每个用例在原有数据断言之上追加以下**过程断言**（任一违例即 FAIL，报告中标注违例类别）：

| # | 断言 | 判定依据（隔离 agent 实例会话事件流水） |
|---|------|--------------------------------------|
| 1 | **按钮 ≤ 1 次/用例** | `[C4_BUTTON_CONFIRM]` 发送次数 ≤ 1；用例脚本显式声明的多轮确认（如批量删除逐台确认）除外 |
