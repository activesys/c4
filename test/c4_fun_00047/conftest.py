...
# ──────────────────────────────────────────────
#  Raw TCP fragmented packet helper
# ──────────────────────────────────────────────

FLAG_V211 = b"ASFPV211"
ATTR_KEY_SEQUENCE = 0x01
ATTR_SAME_DATA_TYPE = 0x02
ATTR_SAME_TIMESTAMP = 0x04


def _encode_asfpv211_packet(addr_start, count, data_type, val_start, ts, attr, mutable_type):
    """构建一个 ASFPV211 数据包（字节串），所有属性开关按 attr 控制。

    参数：
        addr_start:  起始地址
        count:       数据项个数
        data_type:   数据类型 (4=UINT16, 10=FLOAT32)
        val_start:   起始值
        ts:          时间戳
        attr:        属性位掩码 (bit0=KEY_SEQUENCE, bit1=SAME_DATA_TYPE, bit2=SAME_TIMESTAMP)
        mutable_type:可变段中的 type 值

    返回：完整的 ASFPV211 包字节串
    """
    import struct as _struct

    has_key = bool(attr & ATTR_KEY_SEQUENCE)
    has_type = bool(attr & ATTR_SAME_DATA_TYPE)
    has_ts = bool(attr & ATTR_SAME_TIMESTAMP)
    ...
    if has_type:
        mutable_size += 1  # type
    if has_key:
        mutable_size += 3  # 3-byte addr
    if has_ts:
        mutable_size += 8  # timestamp

    # data size per item
    if has_key and has_type and has_ts and data_type in (0, 15):
        # BIT compression
        data_size = (count + 7) // 8
        compressing = True
    else:
        ...
        if not has_ts:
            item_size += 8
        if data_type in (0, 1, 2, 15):
            item_size += 1
        elif data_type in (3, 4, 9):
            item_size += 2
        elif data_type in (5, 6, 10):
            item_size += 4
        elif data_type in (7, 8, 11):
            item_size += 8
        else:
            item_size += 1  # fallback
        data_size = count * item_size
        compressing = False

    total_len = 16 + mutable_size + data_size
    buf = bytearray(total_len)

    # flag (8B)
    buf[0:8] = FLAG_V211
    # length_low (2B), count (2B)
    _struct.pack_into("!HH", buf, 8, total_len & 0xFFFF, count)
    # attribute (4B): (length_high << 16) | attr
    length_high = (total_len >> 16) & 0xFFFF
    _struct.pack_into("!I", buf, 12, (length_high << 16) | attr)

    pos = 16
    # mutable
    if has_type:
        buf[pos] = mutable_type
        pos += 1
    if has_key:
        buf[pos] = (addr_start >> 16) & 0xFF
        buf[pos + 1] = (addr_start >> 8) & 0xFF
        buf[pos + 2] = addr_start & 0xFF
        pos += 3
    if has_ts:
        _struct.pack_into("!Q", buf, pos, ts)
        pos += 8
    ...
    if compressing:
        ...
        for i in range(count):
            ...
            if not has_key:
                ...
                buf[pos + 1] = (addr >> 8) & 0xFF
                buf[pos + 2] = addr & 0xFF
                pos += 3
            if not has_ts:
                _struct.pack_into("!Q", buf, pos, ts)
                pos += 8
            # value — UINT16 for simplicity
            _struct.pack_into("!H", buf, pos, (val_start + i) & 0xFFFF)
            pos += 2

    return bytes(buf)


def send_fragmented_packet(port, addr_start, count, data_type, val_start, ts, split_at):
    """发送被 TCP 分片的 ASFPV211 数据包到指定端口。

    构建一个完整包后，在 split_at 字节处切断，分两次 write 发送，
    模拟 TCP 流分片场景以测试解析器的残包缓冲能力。
    ...
    参数：
        ...
        val_start:  起始值
        ts:         时间戳
        split_at:   分割点（字节偏移，必须在 header(16) 之后）

    返回：(total_sent, partition1_len, partition2_len)
    """
    pkt = _encode_asfpv211_packet(
        addr_start, count, data_type, val_start, ts,
        attr=ATTR_KEY_SEQUENCE | ATTR_SAME_DATA_TYPE | ATTR_SAME_TIMESTAMP,
        mutable_type=data_type,
    )

    if split_at < 18 or split_at >= len(pkt):
        raise ValueError(f"split_at={split_at} must be in (18, {len(pkt)})")
    ...
    try:
        ...
        part2 = pkt[split_at:]
        sock.sendall(part1)
        time.sleep(0.05)  # 让 OS 有机会将 part1 作为独立 TCP segment 发出
        sock.sendall(part2)
    finally:
        sock.close()

    return len(pkt), len(part1), len(part2)
