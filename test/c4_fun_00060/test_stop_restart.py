...
class TestStopRestart:
    ...
    def test_tc4_simple_restart(
        ...
        操作：stop → start → 确认 port 19900 重新连接。
        """
        iid = "c4_tc4004"
        isolated_shm(iid)
        port = 19900
    ...
    def test_tc8_config_change_on_restart(
        ...
        操作：
          ...
          3. 在 127.0.0.2:9901 启动 asfp2_server
             （若 127.0.0.2 不可用则用其他可达的 loopback 别名）
          4. start（新 config_path）
        验证：新地址 127.0.0.2:9901 收到连接，旧地址 127.0.0.1:19900 无连接。
        """
        iid = "c4_tc8008"
        isolated_shm(iid)
        old_port = 19900
    ...
    def test_tc9_error_recovery(
        ...
        """TC9: 不可达目标不阻塞启动；修正配置后重试，无需重启 SUT 进程。

        前置：标准配置。SUT 已 start。
        操作：
          1. stop
          2. 配置改为不可达目标（192.0.2.1:9999）→ adjust_shm
          3. start → success（实例进入 T0 后台重拨，§T0）
          4. stop
          5. 配置改回标准配置 → adjust_shm
          6. start → success，port 19900 连接恢复。
        """
        iid = "c4_tc9009"
        isolated_shm(iid)
        port = 19900
        ...
        try:
            ...
            try:
                # 操作 3: start → success（T0 后台重拨，不阻塞启动）
                resp = client.call_tool(
