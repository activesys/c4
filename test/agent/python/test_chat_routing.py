...
class TestChatRouting:
    ...
    def test_doc_parser_triggered_by_upload_xlsx(
        ...
        assert "无法解析" not in text, (
            f"doc-parser should return parsed content, got: {text[:300]}"
        )
        # 点表中的数据应出现在文本中
...
class TestDocParser:
    ...
    def test_parse_valid_xlsx(self, chat, agent, tmp_path):
        """4.3.1: 上传合法 xlsx 点表 → 解析结果含设备名、数据点"""
        ...
        assert "华能阿拉善" in text or "风机" in text, (
            f"Expected device name in doc-parser result, got: {text[:500]}"
        )
        # 结构化数据在 tool_result 中，文本至少含点数和设备名
