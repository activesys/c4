...
| 4.3.11 | 点名缺失 → 追问，仍缺即拒绝（禁止编造/生成） | 点表仅地址无点名（func_test_case 用例 8） | 逐点询问点名；用户仍不提供 → **拒绝本次接入**（2026-09-23 裁定：确定性生成名已废止），禁止「点1000」式编造 |
| 4.3.12 | 同回合拓扑序 | 一条消息同时提供协议+点表 | 阶段 3 消费同回合阶段 2 产物（1→2→3→4、5→6→7 拓扑序合并）——缺口不重复追问 |

### 4.4 方案层（阶段 8，纯代码）+ 协议锁 + 失效传播

**被测对象**：方案层装配（agent.md §3.2.0.1——纯代码：合并原料 + abbr + L1 结构
校验 + L2 validate_points → AccessPlan + button_arm）与协议逐侧锁（§2.4.1）。
点名相关规格（2026-09-23 裁定）：采集点名必填（缺→问→仍缺→拒绝接入，无生成名）；id 由
点名原文/提取层翻译派生；转发点无自身 name，展示按序引用采集点点名。
...
| # | 用例 | 输入 | 预期 |
|---|------|------|------|
| 4.4.1 | 缺口闭合 → 自动装配 + button_arm | 单消息提供全部信息（func_test_case 用例 1 形态） | 对话含方案要素（设备/协议/端口/「地址 ↔ 点名」映射）；有转发时转发点逐条展示与采集点的对应（`采集 <地址>（<点名>） → 转发 <地址>`）；SSE 收到 `button_arm`；确认按钮渲染 |
...
| 4.4.8 | **转发点表数量不等 → 拦截** | 采集 10 点 + 转发点表仅 5 个（如 5000~5004） | button_arm **不出现**；Agent 指出数量不一致、要求按序一一对应并追问修正；config.json 无写入（2026-09-23 裁定） |
