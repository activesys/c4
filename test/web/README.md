...
| # | web.md 声明 | 契约断言 |
|---|------------|---------|
| 6.1 | 上传解析为确定性步骤（§3.2.1/§3.2.2，2026-09-23） | 上传可解析 .txt：流式返回解析文本，流中**不产出** `tool_call` 事件（无解析工具卡片） |
| 6.2 | `upload` 的 `done` 事件携带 `conversationId`（§3.2.1/§3.2.2） | `done` 事件带 `conversationId` 且与 `X-Conversation-Id` 头一致；非 done 事件不带 |
| 6.3 | 后端不产出 `interrupt` 事件（§1.3） | 完整接入对话流中，SSE 事件**不**出现 `interrupt` |
| 6.4 | 缺口聚合提问回合以 `done` 收尾（§4.2，2026-09-23） | site 未绑定时首条接入消息 → 聚合提问文本（含场站名称与缩写缺口），回合以 `done` 收尾（旧「早退无 done」分支已废除） |
